...
def save(file_name, ep, model, optimizer, scheduler, total_it, name, ema_ESMotion=None):
    state = {
    ...
    if ema_ESMotion is not None:
        ESMotion_state_dict = model.state_dict()
        ema_ESMotion_state_dict = ema_ESMotion.state_dict()
        clip_weights = [e for e in ESMotion_state_dict.keys() if e.startswith('clip_model.')]
        for e in clip_weights:
            del ESMotion_state_dict[e]
            del ema_ESMotion_state_dict[e]
        state[name] = ESMotion_state_dict
        state["ema_ESMotion"] = ema_ESMotion_state_dict
