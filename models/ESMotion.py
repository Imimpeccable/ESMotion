...
class ESMotion(nn.Module):
    def __init__(self, ae_dim, cond_mode, latent_dim=256, ff_size=1024, num_layers=8,
        ...
        if self.cond_mode == 'text':
            self.cond_seq_dim = 512 # CLIP ViT-B/32 
            self.cond_emb = nn.Linear(self.clip_dim, self.latent_dim)
        elif self.cond_mode == 'action':
            assert 'num_actions' in kargs
            self.num_actions = kargs.get('num_actions', 1)
            self.encode_action = partial(F.one_hot, num_classes=self.num_actions)
            self.cond_seq_dim = self.num_actions
            self.cond_emb = nn.Linear(self.num_actions, self.latent_dim)
        elif self.cond_mode == 'uncond':
            self.cond_seq_dim = self.latent_dim
        ...
        self.MARTransformer = nn.ModuleList([
            MARTransBlock(
                self.latent_dim, self.cond_seq_dim, num_heads, mlp_size=ff_size, drop_out=self.dropout
        ...
        # --------------------------------------------------------------------------
        # DiffMLPs
        print(f'Loading DiffMLPs ({diffmlps_model})...')
    ...
    def encode_text(self, raw_text):
        ...
        x = x.permute(1, 0, 2)
        seq_feat = self.clip_model.ln_final(x).type(self.clip_model.dtype).float()
        
        return feat_clip_text, seq_feat

    def mask_cond(self, cond, cond_seq, force_mask=False):
        bs, d = cond.shape
        if force_mask:
            return torch.zeros_like(cond), torch.zeros_like(cond_seq)
        elif self.training and self.cond_drop_prob > 0.:
            mask = torch.bernoulli(torch.ones(bs, device=cond.device) * self.cond_drop_prob).view(bs, 1)
            cond = cond * (1. - mask)
            
            mask_seq = mask.unsqueeze(2) 
            cond_seq = cond_seq * (1. - mask_seq)
            return cond, cond_seq
        else:
            return cond, cond_seq

    def forward(self, latents, cond, cond_seq, padding_mask, force_mask=False, mask=None):
        cond, cond_seq = self.mask_cond(cond, cond_seq, force_mask=force_mask)
        ...
        for block in self.MARTransformer:
            x = block(x, cond, cond_seq, padding_mask)
    ...
    def forward_loss(self, latents, y, m_lens):
        ...
        if self.cond_mode == 'text':
            with torch.no_grad():
                cond_vector, cond_seq = self.encode_text(y)
        elif self.cond_mode == 'action':
            cond_vector = self.encode_action(y).to(device).float()
            cond_seq = cond_vector.unsqueeze(1) 
        elif self.cond_mode == 'uncond':
            cond_vector = torch.zeros(b, self.latent_dim).float().to(device)
            cond_seq = torch.zeros(b, 1, self.cond_seq_dim).float().to(device)
            force_mask = True
        else:
        ...
        z = self.forward(input_latents, cond_vector, cond_seq, ~non_pad_mask, force_mask)

        target = target.reshape(b * l, -1).repeat(self.diffmlps_batch_mul, 1)
        z = z.reshape(b * l, -1).repeat(self.diffmlps_batch_mul, 1)
        mask_flat = mask.reshape(b * l).repeat(self.diffmlps_batch_mul)

        target = target[mask_flat]
        z = z[mask_flat]

        loss = self.DiffMLPs(target=target, z=z)
        return loss

    def forward_with_CFG(self, latents, cond_vector, cond_seq, padding_mask, cfg=3, mask=None, force_mask=False, hard_pseudo_reorder=False, temperature=1.0):
        ...
        if force_mask:
            return self.forward(latents, cond_vector, cond_seq, padding_mask, force_mask=True, mask=reorder_mask)

        logits = self.forward(latents, cond_vector, cond_seq, padding_mask, mask=reorder_mask)
        
        if cfg != 1:
            aux_logits = self.forward(latents, cond_vector, cond_seq, padding_mask, force_mask=True, mask=reorder_mask)
        ...
        if mask is not None:
            mask2 = torch.cat([mask, mask], dim=0).reshape(b * l) if cfg != 1 else mask.reshape(b * l)
            mixed_logits = (mixed_logits.reshape(b * l, d))[mask2]
        else:
            mixed_logits = mixed_logits.reshape(b * l, d)

        output = self.DiffMLPs.sample(mixed_logits, temperature=temperature, cfg=cfg)

        if cfg != 1:
            scaled_logits, _ = output.chunk(2, dim=0) if output.shape[0] == mixed_logits.shape[0] else (output, None)
            scaled_logits = output 
        else:
            scaled_logits = output

        if mask is not None:
            orig_b = b // 2 if cfg != 1 else b
            latents = latents.reshape(orig_b * l, self.ae_dim)
            latents[mask.reshape(orig_b * l)] = scaled_logits
            scaled_logits = latents.reshape(orig_b, l, self.ae_dim)
        else:
             scaled_logits = scaled_logits.reshape(b // 2 if cfg!=1 else b, l, self.ae_dim)

        return scaled_logits

    @torch.no_grad()
    @eval_decorator
    def generate(self, conds, m_lens, timesteps: int, cond_scale: int, temperature=1, force_mask=False, hard_pseudo_reorder=False):
        device = next(self.parameters()).device
        l = max(m_lens)
        b = len(m_lens)

        if self.cond_mode == 'text':
            cond_vector, cond_seq = self.encode_text(conds)
        elif self.cond_mode == 'action':
        ...
        for timestep, steps_until_x0 in zip(torch.linspace(0, 1, timesteps, device=device), reversed(range(timesteps))):
            ...
            logits = self.forward_with_CFG(latents, cond_vector=cond_vector, cond_seq=cond_seq, padding_mask=padding_mask,
                                           cfg=cond_scale, mask=is_mask, force_mask=force_mask, 
                                           hard_pseudo_reorder=hard_pseudo_reorder, temperature=temperature)
    ...
    @torch.no_grad()
    @eval_decorator
    def edit(self, conds, latents, m_lens, timesteps: int, cond_scale: int, temperature=1, force_mask=False, edit_mask=None, padding_mask=None, hard_pseudo_reorder=False):
        device = next(self.parameters()).device
        l = latents.shape[-1]
        ...
        if self.cond_mode == 'text':
            ...
            cond_seq = torch.zeros(1, 1, self.cond_seq_dim).float().to(device)
        else:
            raise NotImplementedError("Unsupported condition mode!!!")
        ...
        if edit_mask is None:
            ...
            latents = torch.where(
                ...
                torch.zeros(latents.shape[0], l, self.ae_dim).to(device),
...
ESMotion_models = {
    "ESMotion-Score-XL": esmotion_score_xl,
    "ESMotion-SiT-XL": esmotion_score_xl,
    "ESMotion": esmotion_score_xl,
}
...
class CrossAttention(nn.Module):
    def __init__(self, embed_dim=512, cond_dim=512, n_head=8, drop_out_rate=0.2):
        super().__init__()
    ...
    def forward(self, x, cond_seq, mask=None):
        B, T, C = x.size()
        B_c, T_c, C_c = cond_seq.size()

        q = self.query(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)
        k = self.key(cond_seq).view(B_c, T_c, self.n_head, C // self.n_head).transpose(1, 2)
        v = self.value(cond_seq).view(B_c, T_c, self.n_head, C // self.n_head).transpose(1, 2)
...
class MARTransBlock(nn.Module):
    def __init__(self, hidden_size, cond_seq_dim, num_heads, mlp_size=1024, drop_out=0.2):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.attn = Attention(hidden_size, num_heads, drop_out_rate=drop_out)

        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.cross_attn = CrossAttention(hidden_size, cond_seq_dim, num_heads, drop_out_rate=drop_out)
    ...
    def forward(self, x, c, cond_seq, padding_mask=None):
        shift_msa, scale_msa, gate_msa, shift_ca, scale_ca, gate_ca, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(9, dim=1)
        x = x + gate_msa.unsqueeze(1) * self.attn(modulate_here(self.norm1(x), shift_msa, scale_msa), mask=padding_mask)
        x = x + gate_ca.unsqueeze(1) * self.cross_attn(modulate_here(self.norm2(x), shift_ca, scale_ca), cond_seq, mask=None)
        x = x + gate_mlp.unsqueeze(1) * self.mlp(modulate_here(self.norm3(x), shift_mlp, scale_mlp))
        return x
